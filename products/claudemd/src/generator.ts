import type { ProjectSignals } from "./detector.js";
...
function generateHeader(answers: UserAnswers): string {
...
function generateWorkflow(answers: UserAnswers): string {
...
function generateCodeStyle(answers: UserAnswers): string {
...
function generateCommonTasks(answers: UserAnswers): string {
  const lines = [
    "## Common Tasks",
    "",
...
function generateDoNotRules(answers: UserAnswers): string {
  const lines = [
    "## Do NOT",
    "",
...
function generateTesting(
  ...
  if (signals.testFrameworks.length > 0) {
    lines.push(
      "",
      `Tests use ${signals.testFrameworks.join(" and ")}. Follow existing test patterns when adding new tests.`
    );
  }
...
function generateAdditionalNotes(answers: UserAnswers): string {
