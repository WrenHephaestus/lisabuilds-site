...
export async function runPrompts(signals) {
    console.log();
    // Project name
    ...
    console.log("  These are boundaries Claude must respect. Think about what would frustrate you.\n");
    const doNotRules = [];
    ...
    const architecture = await input({
        message: "Brief architecture description (e.g., 'React frontend + Express API + PostgreSQL'):",
        default: "",
    });
    // Team context
...
function guessDevCommand(signals) {
    if (signals.packageManager === "pnpm")
        return "pnpm dev";
    if (signals.packageManager === "yarn")
        return "yarn dev";
    if (signals.packageManager === "bun")
        return "bun dev";
    if (signals.languages.includes("Python"))
        return "python main.py";
    if (signals.languages.includes("Go"))
        return "go run .";
    if (signals.languages.includes("Rust"))
        return "cargo run";
    if (signals.languages.includes("Ruby"))
        return "bundle exec rails server";
    return "npm run dev";
}
function guessBuildCommand(signals) {
    if (signals.packageManager === "pnpm")
        return "pnpm build";
    if (signals.packageManager === "yarn")
        return "yarn build";
...
function guessTestCommand(signals) {
    if (signals.testFrameworks.includes("Vitest"))
        return "npx vitest";
    if (signals.testFrameworks.includes("Jest"))
        return "npx jest";
    if (signals.testFrameworks.includes("pytest"))
        return "pytest";
    if (signals.testFrameworks.includes("RSpec"))
