        ...
        parse: (content) => {
            try {
                ...
                if (signals.frameworks)
                    signals.frameworks.push(...frameworks);
                else
    ...
    // Python
    {
        ...
        parse: (content) => {
            ...
            if (content.includes("pytest"))
                signals.testFrameworks = ["pytest"];
            if (content.includes("poetry"))
                signals.buildTools = ["Poetry"];
            if (content.includes("hatch"))
                signals.buildTools = ["Hatch"];
            if (frameworks.length)
                signals.frameworks = frameworks;
            const nameMatch = content.match(/^name\s*=\s*"([^"]+)"/m);
            if (nameMatch)
                signals.projectName = nameMatch[1];
            const descMatch = content.match(/^description\s*=\s*"([^"]+)"/m);
            if (descMatch)
                signals.projectDescription = descMatch[1];
            return signals;
        },
    },
    {
    ...
    // Java / Kotlin
    {
        ...
        parse: (content) => {
            ...
            if (content.includes("kotlin"))
                signals.languages = ["Kotlin"];
            if (content.includes("spring"))
                signals.frameworks = ["Spring Boot"];
            return signals;
        },
    },
    {
...
function detectKeyDirectories(dir) {
    const interesting = [
        "src",
        "lib",
        "app",
        ...
        try {
            ...
            for (const entry of common) {
                if (entries.includes(entry)) {
                    const relative = checkDir === dir ? entry : join(basename(checkDir), entry);
                    found.push(relative);
                }
            }
        }
